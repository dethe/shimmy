...
class ui {
  ...
  static drawRotationAnchor() {
    this.ctx.lineWidth = 3;
    this.ctx.beginPath();
    let off = this.offset * 0.05;
    this.ctx.arc(this.tool.anchorX, this.tool.anchorY, 4, 0 + off, 1.75 * Math.PI + off);
    this.ctx.stroke();
    this.ctx.beginPath();
    this.ctx.arc(
      this.tool.anchorX,
  ...
  drawAnts() {
    ...
    this.ctx.beginPath();
    this.ctx.moveTo(this.tool.anchorX, this.tool.anchorY);
    this.ctx.lineTo(this.tool.mouseX, this.tool.mouseY);
    this.ctx.stroke();
    this.ctx.restore();
  }
